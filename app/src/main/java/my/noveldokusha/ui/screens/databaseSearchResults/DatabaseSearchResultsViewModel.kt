...
interface DatabaseSearchResultsStateBundle {
    ...
    val input
        get() = when (this.searchMode) {
            DatabaseSearchResultsActivity.SearchMode.Text::class.simpleName -> DatabaseSearchResultsActivity.SearchMode.Text(
                text = text
            )
...
class DatabaseSearchResultsViewModel @Inject constructor(
    ...
    val fetchIterator = PagedListIteratorState(viewModelScope) { index ->
        when (val input = this.input) {
            is DatabaseSearchResultsActivity.SearchMode.Text -> database.getSearch(
                index,
                input.text
            )
            is DatabaseSearchResultsActivity.SearchMode.Genres -> database.getSearchAdvanced(
                index,
                input.genresIncludeId,
                input.genresExcludeId
            )
            is DatabaseSearchResultsActivity.SearchMode.AuthorSeries -> database.getSearchAuthorSeries(
                index,
                input.urlAuthorPage
            )
        }
